package ressource;

public class FrameConstants {
	
	public static String[] TABLE_HEADER = {"Host", "Port", "Active", "Timestamp"};
	public static String FRAME_TITLE = "P0rtSc4nn3r";
	
	public static String BTN_START = "Start";
	public static String BTN_STOP = "Stop";
	public static String BTN_CLEAR = "Clear";
	public static String BTN_SAVE = "Save";
	public static String BTN_COPY = "Copy";
	
	public static String TEXT_INVALID = "Invalid Usage!";
	public static String TEXT_INVALID_IP = "Invalid IP Address!";
	public static String TEXT_INVALID_MIN_PORT = "Invalid Min Port!";
	public static String TEXT_INVALID_MAX_PORT = "Invalid Max Port!";
	public static String TEXT_HOST_UNREACHABLE = "The Target Host Is Unreachable!";
	public static String TEXT_FILE_SAVED = "The File Has Been Saved.";
	public static String TEXT_FILE_SAVE_FAILED = "The File Has Not Been Saved.";
	public static String TEXT_NOTHING_TO_SAVE = "Nothing To Save.";
	public static String TEXT_COPIED_TO_CLIPBOARD = "Copied The Text To Your Clipboard.";
	
	public static String DIALOG_FINISHED_TITLE = "Finished";
	
}
